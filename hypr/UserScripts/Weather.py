    ...
    if code == -1:
        try:
            ...
            codes: List[Optional[int]] = (
                safe_get(forecast, "hourly", "weather_code", default=[]) or []
            )
            cur_time: Optional[str] = safe_get(forecast, "current", "time")
            ...
            if cur_time and times:
                try:
                    ...
                    for t in times:
                        try:
                            diffs.append(
                                abs((datetime.fromisoformat(t) - ct).total_seconds())
                            )
                        except Exception:
            ...
            if isinstance(hcode, int):
                code = hcode
                log_debug(
                    f"Fallback hourly weather_code used: code={code} idx={idx} cur_time={cur_time}"
                )
        except Exception as e:
